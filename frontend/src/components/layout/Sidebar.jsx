...
export default function Sidebar({ patients, reports, selectedId, onSelect, activeTab, onNavigate }) {
  ...
  // Matches against the full UUID (so a pasted ID always works), the displayed "#XXXX" label, and
  // the optional demo `label` field (so "cardiac" finds "Demo - Cardiac Stress") -- most patients
  // have no real name to search by otherwise. Case-insensitive, dashes stripped in the ID match so
  // "1a8f" and "1A8F8EA9-..." both hit.
  const filtered = useMemo(() => {
    ...
    return patients.filter(
      (p) => p.id.replace(/-/g, '').toLowerCase().includes(qNoDash) || p.label?.toLowerCase().includes(q)
    )
  }, [patients, query])
        ...
        {filtered?.map((p) => {
          ...
          return (
            <div key={p.id} className={`prow ${p.id === selectedId ? 'sel' : ''}`} onClick={() => onSelect(p.id)}>
              ...
              <div>
                <div className="pname">{p.label || avatar.label}</div>
