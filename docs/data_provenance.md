...
| NYHA class default for live ML Model 1 inference before any assessment exists (`src/scenario_classifier/features.py`'s `build_inference_features`) | Class I | — | — | **assumed_default** — a brand-new patient has no NYHA class yet (it's what `staging.py` *computes*, downstream of this feature vector); defaults to the most benign class rather than guessing, consistent with not overstating risk on missing data. |
